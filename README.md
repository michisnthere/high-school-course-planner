...
Use the new Phase 1 extraction pipeline scripts from the project root.

- Classify key pages:
  ```bash
  python scripts/check_pages.py --pages 17 18 22
  ```
- Extract page 18 text and metadata:
  ```bash
  python scripts/extract_page.py --page 18
  ```
- Run AI extraction only after setting `OPENAI_API_KEY` and passing `--ai`:
  ```bash
