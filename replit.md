...
2. **Node.js/Prisma import pipeline** — Takes the cleaned `extractor/output/academic-data.json` and imports it into PostgreSQL with a normalized Division → Department → Course → CourseOption → CourseOffering hierarchy.
...
1. `extractor/output/academic-data.json` — Combined, cleaned catalog JSON produced by the extraction pipeline (input to import script)
2. `scripts/import_courses.ts` — Reads `extractor/output/academic-data.json` and upserts divisions, departments, courses, options, and offerings into PostgreSQL
